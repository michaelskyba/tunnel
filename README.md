...
If the deck file was ``/home/michael/decks/geography`` and ``$TMPDIR`` was
unset, the retry file's path would be
``/tmp/tunnel/home/michael/decks/geography``. We need to have the same chain of
directories because a user could have different deck files with the same
filename being reviewed.
